...
This is designed to be run overnight. It supports:
  ...
  - final refit on the best hyperparameters and a submission.csv
...
RESULTS_JSONL = BASE / "lira_search_results-9.jsonl"
RESULTS_CSV = BASE / "lira_search_results-9.csv"
BEST_CONFIG_JSON = BASE / "best_lira_config-9.json"
...
# LiRA statistics
# ============================================================
def true_class_scalar_from_logits(logits: torch.Tensor, labels: torch.Tensor, stat_mode: str) -> torch.Tensor:
    """
...
def collect_scalar_scores(model, dataset, batch_size: int, stat_mode: str) -> np.ndarray:
    model.eval()
...
def collect_reference_matrix(reference_models, dataset, batch_size: int, stat_mode: str):
    n = len(dataset)
    ...
    for i, ref in enumerate(reference_models):
        ...
        scores[i] = collect_scalar_scores(ref["model"], dataset, batch_size=batch_size, stat_mode=stat_mode)
...
def run_trial(pub_ds, target_model, cfg: SearchConfig, seed: int, batch_size: int):
    ...
    ref_scores_pub, ref_in_mask = collect_reference_matrix(
        refs, pub_ds, batch_size=batch_size, stat_mode=cfg.stat_mode
    )
    ...
    target_scores_pub = collect_scalar_scores(target_model, pub_ds, batch_size=batch_size, stat_mode=cfg.stat_mode)
    ...
    # 6) Private scores is not needed for this attack, so we skip it

    # cleanup
    del refs, ref_scores_pub, ref_in_mask, target_scores_pub, X_pub, calibrator
    gc.collect()
...
def build_search_space(args) -> List[SearchConfig]:
    ...
    for combo in itertools.product(
        ...
        grid.append(SearchConfig(
            ...
            feature_mode=str(combo[6]),
            cal_C=float(combo[7]),
            cal_class_weight=str(combo[8]),
            ridge_alpha=float(combo[9]),
        ))
    return grid
...
def main():
    ...
    parser.add_argument("--mode", choices=["random", "grid"], default="grid")
    parser.add_argument("--max-trials", type=int, default=48)
    parser.add_argument("--seed", type=int, default=676677)
        ...
        if RESULTS_JSONL.exists():
            ...
            try:
                ...
                if len(df) > 0:
                    ...
                    best_cfg = SearchConfig(**best_json)
